...
vformat_t aml_get_vformat(AVCodecContext *avctx)
{
  ...
  switch (avctx->codec_id)
  {
    ...
    // AV_CODEC_ID_MPEG2VIDEO_XVMC removed, folded into MPEG2VIDEO
...
//      break;
  ...
  // codec_tag is frequently 0 (MKV / MPEG-TS / RTSP / raw ES), so the tag
  // switch above leaves dec_type UNKNOW. Derive it from codec_id instead,
  // which is always populated by the demuxer/parser.
  if (dec_type == VIDEO_DEC_FORMAT_UNKNOW)
  {
    av_log(avctx, AV_LOG_DEBUG,
           "codec tag 0x%x not recognised, deriving vdec type from codec id %d\n",
           avctx->codec_tag, avctx->codec_id);
    switch (avctx->codec_id)
    {
      case AV_CODEC_ID_HEVC:
        dec_type = VIDEO_DEC_FORMAT_HEVC;
      ...
      case AV_CODEC_ID_H264:
        dec_type = (aml_get_vformat(avctx) == VFORMAT_H264_4K2K)
                 ? VIDEO_DEC_FORMAT_H264_4K2K : VIDEO_DEC_FORMAT_H264;
      break;
      ...
      // MS-MPEG4 v1/v2/v3 use the DivX 3.11 hardware path
      case AV_CODEC_ID_MSMPEG4V1:
      case AV_CODEC_ID_MSMPEG4V2:
      case AV_CODEC_ID_MSMPEG4V3:
        dec_type = VIDEO_DEC_FORMAT_MPEG4_3;
      ...
      // RealVideo: RV30/G2 -> REAL_8, RV40 (RMVB) -> REAL_9
      case AV_CODEC_ID_RV30:
        dec_type = VIDEO_DEC_FORMAT_REAL_8;
      break;
      case AV_CODEC_ID_RV40:
      ...
      // generic MPEG-4 ASP (Xvid / DivX 4-6 / MP4V) and FLV1
      case AV_CODEC_ID_MPEG4:
      case AV_CODEC_ID_FLV1:
        dec_type = VIDEO_DEC_FORMAT_MPEG4_5;
      break;

      // MPEG-1/2 ignore am_sysinfo.format; UNKNOW (0) is an accepted value
      // there (and is distinct from the -1 "could not determine" sentinel)
      case AV_CODEC_ID_MPEG1VIDEO:
      case AV_CODEC_ID_MPEG2VIDEO:
        dec_type = VIDEO_DEC_FORMAT_UNKNOW;
      break;
      ...
      default:
        av_log(avctx, AV_LOG_ERROR,
               "Failed to derive vdec type for codec id %d\n", avctx->codec_id);
      break;
    }
  }
